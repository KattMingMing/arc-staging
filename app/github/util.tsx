import { GitHubBlobUrl, GitHubMode, GitHubPullUrl, GitHubRepositoryUrl, GitHubURL } from '../github/index'
import { CodeCell, DiffRepoRev, DiffResolvedRevSpec, MaybeDiffSpec } from '../repo/index'
...
export function getDiffRepoRev(): DiffRepoRev | null {
    const { repoPath, isDelta, isPullRequest, isCommit } = parseURL()
    if (!isDelta) {
        return null
    }
...
export function parseURL(loc: Location = window.location): GitHubURL {
    ...
    return { user, repoName, rev, filePath, repoPath, isDelta, isPullRequest, position, isCommit, isCodePage }
}
